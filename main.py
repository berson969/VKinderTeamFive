...
from database import add_whitelist, add_blacklist, choose_favorites, check_users_in_persons
from bot_auth import Auth


def keyboard_creator(flag=None):
    """
...
def edit_msg(peer_id: int, conversation_message_id: int, message=None, keyboard=None):
    """
...
def _iter_selected(dict_info):
    ...
    :param dict_info: информация о пользователе
    ...
    selected_without_repeat = check_users_in_persons(dict_info, selected)
    count = len(selected_without_repeat)
    _selected = iter(selected_without_repeat)
...
def show_selected(vk_id: int, _selected):
    """
            ...
            работает за счет итератора self.selected,
            в конце итерирования возвращает  False
...
def main():
    ...
    """
    global user_dict_info, selected_dict_info, _selected
    # while True:
    #     try:
    ...
    for event in longpoll.listen():
        ...
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            # Запрос возраста
                ...
                # print(event.obj)
                write_msg(event.message['from_id'], 'Извините, не поняла Вас...', [])
            ...
            # Запрос на выход
                ...
                write_msg(event.obj.peer_id, 'До новых встреч!!', keyboard_creator())
